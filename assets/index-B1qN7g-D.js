(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=class{constructor(){this.flightApiUrl=`https://api.aviationstack.com/v1/flights`,this.weatherApiUrl=`https://avwx.rest/api/metar`,this.flightApiKey=``,this.weatherApiKey=``}async getAirportWeather(e){let t=e.trim().toUpperCase();if(t.length===3&&(t=t===`RAI`?`GVNP`:t===`LIS`?`LPPT`:t===`SID`?`GVAC`:`G${t}`),!this.weatherApiKey)return this._getSandboxWeather(t);try{let e=await fetch(`${this.weatherApiUrl}/${t}`,{headers:{Authorization:`BEARER ${this.weatherApiKey}`}});if(!e.ok)throw Error(`AVWX Telemetry HTTP Error: ${e.status}`);return await e.json()}catch{return console.warn(`AVWX API offline or throttled. Engaging local meteorological sandbox for ${t}.`),this._getSandboxWeather(t)}}async getAirportFlights(e,t=`departure`){let n=e.trim().toUpperCase().substring(0,3);if(!this.flightApiKey)return this._getSandboxFlights(n,t);try{let e=`${this.flightApiUrl}?access_key=${this.flightApiKey}&${t}_iata=${n}&limit=20`,r=await fetch(e);if(!r.ok)throw Error(`Aviationstack HTTP Error: ${r.status}`);return(await r.json()).data||[]}catch{return console.warn(`Aviationstack API throttled. Engaging local flight operations sandbox for ${n}.`),this._getSandboxFlights(n,t)}}_getSandboxWeather(e){return{station:e,raw:`${e} 142200Z 21012KT 9999 FEW018 BKN250 24/18 Q1014 NOSIG`,flight_rules:`VFR`,temperature:{value:24},dewpoint:{value:18},wind_speed:{value:12},wind_direction:{value:210},visibility:{value:10},altimeter:{value:1014},clouds:[{repr:`FEW`,altitude:18},{repr:`BKN`,altitude:250}]}}_getSandboxFlights(e,t){let n=t===`departure`;return[{flight_date:`2026-06-14`,flight_status:`active`,flight:{number:`VR601`},airline:{name:`Cabo Verde Airlines`},departure:{airport:`Nelson Mandela Intl`,iata:`RAI`,icao:`GVNP`,terminal:`T1`,gate:`3`,delay:0},arrival:{airport:`Lisbon Humberto Delgado`,iata:`LIS`,icao:`LPPT`,terminal:`T1`,gate:`A12`,delay:0}},{flight_date:`2026-06-14`,flight_status:`delayed`,flight:{number:`TP1492`},airline:{name:`TAP Air Portugal`},departure:{airport:`Nelson Mandela Intl`,iata:`RAI`,icao:`GVNP`,terminal:`T1`,gate:`1`,delay:55},arrival:{airport:`Lisbon Humberto Delgado`,iata:`LIS`,icao:`LPPT`,terminal:`T1`,gate:`B04`,delay:45}},{flight_date:`2026-06-14`,flight_status:`scheduled`,flight:{number:`AT931`},airline:{name:`Royal Air Maroc`},departure:{airport:`Mohammed V Intl`,iata:`CMN`,icao:`GMMN`,terminal:`T2`,gate:`F2`,delay:10},arrival:{airport:`Nelson Mandela Intl`,iata:`RAI`,icao:`GVNP`,terminal:`T1`,gate:`2`,delay:0}}].filter(t=>n?t.departure.iata===e:t.arrival.iata===e)}},t=class{constructor(e,t){this.listContainer=document.getElementById(e),this.searchForm=document.getElementById(t),this.onSearchCallback=null}init(e){this.onSearchCallback=e,this.searchForm&&this.searchForm.addEventListener(`submit`,e=>{e.preventDefault();let t=this.searchForm.querySelector(`input[type="text"]`);t&&t.value.trim()!==``&&(this.showLoading(),this.onSearchCallback(t.value.trim().toUpperCase()))})}showLoading(){this.listContainer.innerHTML=`
    ...
    `}},r=new e,i=new t(`dashboard-content`,`search-form`),a=new n(`weather-container`),o=[],s=`departure`;async function c(e){let t=document.getElementById(`dashboard-content`);try{let t=await r.getAirportWeather(e);a.render(t),o=await r.getAirportFlights(e,s),i.renderFlights(o),document.getElementById(`filter-controls`)?.classList.remove(`hidden`)}catch(n){console.error(`Erro ao processar dados aeronáuticos:`,n),t.innerHTML=`
